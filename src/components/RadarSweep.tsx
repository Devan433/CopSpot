export default function RadarSweep({ onClick }: { onClick?: () => void }) {
  return (
    <div onClick={onClick} className="absolute bottom-16 right-4 z-10 w-20 h-20 md:w-28 md:h-28 rounded-full border-2 border-[#14375b] bg-transparent flex items-center justify-center overflow-hidden pointer-events-auto cursor-pointer hover:scale-105 transition-transform">
      
      {/* Radar Grid Lines */}
      <div className="absolute inset-0 rounded-full border border-white/20"></div>
      <div className="absolute inset-4 rounded-full border border-white/20"></div>
      <div className="absolute inset-8 rounded-full border border-white/20 hidden md:block"></div>
